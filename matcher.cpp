...
void setup_database() {
    ...
    execute_sql(db, "CREATE TABLE Resources (resource_id INTEGER PRIMARY KEY AUTOINCREMENT, resource_name TEXT UNIQUE);");
    ...
    execute_sql(db, "INSERT INTO Tasks (project_id, task_name, required_skill, duration_hours, schedule_from, schedule_to, status) VALUES (1, 'Setup Database', 'SQL', 40, '2025-08-06', '2025-08-11'), (2, 'Design Database Schema', 'Mongo DB', 24, '2025-08-06', '2025-08-09');");
    execute_sql(db, "INSERT INTO Resources (resource_name) VALUES ('Ram'), ('Shyam'), ('Kiran'), ('Dhina');");
...
void allocate_task(const std::string& project_name, const std::string& task_name, const std::string& skill, int duration_hours, int manual_resource_id = -1) {
    sqlite3* db;
    sqlite3_stmt* stmt;
    json result;
    ...
    if (manual_resource_id != -1) {
        sqlite3_prepare_v2(db, "SELECT R.resource_name FROM Resources R JOIN Resource_Skills RS ON R.resource_id = RS.resource_id WHERE R.resource_id = ? AND RS.skill = ?;", -1, &stmt, 0);
        sqlite3_bind_int(stmt, 1, manual_resource_id);
        sqlite3_bind_text(stmt, 2, skill.c_str(), -1, SQLITE_STATIC);
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            best_resource_id = manual_resource_id;
            best_resource_name = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
        } else {
            result["success"] = false;
            result["message"] = "Assignment failed: The selected resource does not have the required skill.";
            sqlite3_finalize(stmt);
            sqlite3_close(db);
        ...
        sqlite3_prepare_v2(db, "SELECT MAX(T.schedule_to) FROM Assignments A JOIN Tasks T ON A.task_id = T.task_id WHERE A.resource_id = ?;", -1, &stmt, 0);
        sqlite3_bind_int(stmt, 1, best_resource_id);
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            const char* last_task_end = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
        ...
        if (best_resource_id == -1) {
            const char* find_least_work_sql = "SELECT R.resource_id, R.resource_name, MAX(T.schedule_to), SUM(T.duration_hours) as total_hours FROM Resources R JOIN Resource_Skills RS ON R.resource_id = RS.resource_id LEFT JOIN Assignments A ON R.resource_id = A.resource_id LEFT JOIN Tasks T ON A.task_id = T.task_id WHERE RS.skill = ? GROUP BY R.resource_id ORDER BY total_hours ASC LIMIT 1;";
    ...
    if (best_resource_id != -1) {
        ...
        sqlite3_prepare_v2(db, "INSERT INTO Tasks (project_id, task_name, required_skill, duration_hours, schedule_from, schedule_to, status) VALUES (?, ?, ?, ?, ?, ?, 'Assigned');", -1, &stmt, 0);
        sqlite3_bind_int(stmt, 1, project_id);
        ...
        result["success"] = false;
        result["message"] = "No suitable resource could be found for this task.";
    }
...
void complete_task(int task_id) {
    ...
    if (resource_id != -1) {
        ...
        result["success"] = false;
        result["message"] = "Could not find an active assignment for this task.";
    }
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (mode == "--init") {
            ...
            if (argc < 6 || argc > 7) {
                std::cerr << "Usage for --allocate: <project_name> <task_name> <skill> <duration_hours> [resource_id]" << std::endl;
                return 1;
            }
            int resource_id = (argc == 7) ? std::stoi(argv[6]) : -1;
            allocate_task(argv[2], argv[3], argv[4], std::stoi(argv[5]), resource_id);
